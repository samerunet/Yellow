...
export default function Feed({ post, API,  users, comments, setPost, username, setComment }) {
	...
	const editPost = (editedPost, postItem) => {
	...
	const EditForm = ({user, postItem, handleEditTitle, handleEditDescription, handleEditImage}) => { if(username.username === user.username ) {
		...
		return <>
				...
				<input type="text" name="title"  placeholder={postItem.title} onChange={handleEditTitle} />
				<br />
				<br />
				<input type="text" name="description"  placeholder={postItem.description} onChange={handleEditDescription}/>
				<button type='submit' className='bg-lime-500 inline-block px-5 py-3 ml-3 text-sm font-lg  rounded-lg' >Edit</button> 
				</form>
